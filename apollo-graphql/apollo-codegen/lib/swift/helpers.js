...
class Helpers {
    ...
    mapExpressionForType(type, isConditional = false, makeExpression, expression, inputTypeName, outputTypeName) {
        ...
        if (type instanceof graphql_1.GraphQLList) {
            if (isOptional) {
                return `${expression}.flatMap { ${makeClosureSignature(this.typeNameFromGraphQLType(type, inputTypeName, false), this.typeNameFromGraphQLType(type, outputTypeName, false))} value.map { ${this.mapExpressionForType(type.ofType, undefined, makeExpression, `${makeClosureSignature(this.typeNameFromGraphQLType(type.ofType, inputTypeName), this.typeNameFromGraphQLType(type.ofType, outputTypeName))} value`, inputTypeName, outputTypeName)} } }`;
            }
            else {
                return `${expression}.map { ${makeClosureSignature(this.typeNameFromGraphQLType(type.ofType, inputTypeName), this.typeNameFromGraphQLType(type.ofType, outputTypeName))} ${this.mapExpressionForType(type.ofType, undefined, makeExpression, 'value', inputTypeName, outputTypeName)} }`;
            }
        }
        else if (isOptional) {
